local tbl = 
{
	...
	skill = 
	{
		...
		{
			check_gp = false,
			func = "---triple hook\nif MadaoFishSk.CheckHookCastOk(\"triple\", self.id, self.ait_id, self.last_cast_time) then\n    return true\nend\nif self.id == 38935 then\n    --Jhinga / Versatile Lure\n    if Player.status == 36 and Player:GetBait() == 29717 then\n        if os.clock() - self.last_cast_time >= 20 then\n            return true\n        end\n    end\nend",
			use = true,
		},
	},
	spear_blank = 
